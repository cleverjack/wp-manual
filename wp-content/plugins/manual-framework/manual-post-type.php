	...
	function manual_faq_post_type() {
		...
		register_post_type( 'manual_faq',
			...
			'labels' => array(
					...
					'singular_name' => esc_html__( $faq_breadcrumb_name, 'manual-framework' ),
			...
			'menu_icon'  => plugin_dir_url( __FILE__ ) . 'admin/includes/favicon.png',
			)
		);	
		flush_rewrite_rules();
	}
...
if ( ! function_exists( 'manual_documentation_post_type' ) ) {
	...
	function manual_documentation_post_type() {
		...
		if( $theme_options['doc-gutenberg-editor-onoff'] == true ) {
...
function manual_kb_post_type() {
	...
	if( $theme_options['kb-gutenberg-editor-onoff'] == true ) {
